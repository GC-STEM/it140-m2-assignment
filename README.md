# IT 140 Module Two: {{Full Activity Title}}
...
To complete this activity, follow the Software Development Life Cycle (SDLC). The top-level steps are listed below. See the [To-Do List](./name_age_todo.md) for detailed, step-by-step instructions.

## Getting Started

1. **Analyze** the [problem and requirements](./analysis/name_age_srs.md) to understand what needs to be done. Record your understanding of the problem and requirements in your [SDW worksheet](./analysis/name_age_sdw.md).

2. **Design** a solution that meets specification. Review the [Software Design Document](./design/name_age_sdd.md), {{along with its | and create a}} [flowchart](./design/name_age.drawio) and low-level [pseudocode](./design/name_age.pseudo).

3. **Construct** a working program in [`name_age.py`](./src/name_age.py) that implements the designed solution.
...
│   ├── name_age_sdw.md    # Software Development Worksheet (SDW) to plan your work
│   └── name_age_srs.md    # Software Requirements Specification (SRS)
│
├── design/                # 2. Design: plan the solution
│   ├── name_age_sdd.md    # Software Design Document (SDD)
│   ├── name_age.drawio    # Flowchart: visual outline of project logic
│   └── name_age.pseudo    # Pseudocode: step-by-step logic before coding
│
├── src/                   # 3. Construction: write the code
│   └── name_age.py        # Python source (src) code file
│
├── tests/                 # 4. Test: check that code meets requirements
│   └── test_name_age.py   # Automated integration tests
│
├── name_age_todo.md       # Step-by-step directions for this activity
